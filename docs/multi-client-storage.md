...
Writes are serialised in-process, reads are disposed, and a full Realm `WriteCopy` backup is reopened read-only before each write. The most recent five verified backups per root are retained under `user://realm-backups/<root hash>/`. Backups have owner-only permissions on Unix. These are **database backups**, not backups of the entire file store. They contain private osu! data; do not attach them to bug reports.
